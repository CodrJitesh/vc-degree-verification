...
const claims = {
    ...
    if (process.env.PRIVADO_ENABLED === 'true' && process.env.ISSUER_NODE_URL) {
      try {
        const issuerNodeRes = await fetch(`${process.env.ISSUER_NODE_URL}/v1/${process.env.ISSUER_DID}/claims`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            // Add authorization if required by Issuer Node
          },
          body: JSON.stringify({
            credentialSchema: process.env.SCHEMA_CONTEXT_URL,
        ...
        const issuerData = await issuerNodeRes.json();
        console.log('[Privado] Issued real Iden3 credential via Issuer Node:', issuerData);
        // Optionally attach the Iden3 credential to our local response
        credential.iden3Credential = issuerData;
      } catch (err) {
        console.error('[Privado] Failed to bridge to Issuer Node:', err.message);
      }
    }
